...
<ThumbWarningAlert>
  <p>Dans cette page nous allons uniquement les mots-clés pour les actions de base fournies par les frameworks (exemple: "clicker", "changer de page"). Car les mots-clés pour les actions personnalisées dépendent de l'application testée et sont donc spécifiques à chaque projet. (exemple: "Se connecter", "Ajouter un produit au panier", etc.)</p>
</ThumbWarningAlert>
...
Only by reading the code we can understand what the test is doing and the steps it is taking. We don't need to know the implementation details of the `cy` object.
