...
<ThumbWarningAlert>
    ...
    il est recommandé d'effectuer le Teardown avant même le setup, afin qu'il soit toujours exécuté, même en cas d'interruption des tests.
</ThumbWarningAlert>

<ThumbUpAlert>
    Cypress permet de continuer la navigation manuellement après l'exécution des tests, tout en conservant le contexte de la page. Cela encourage au développement par les tests, et facilite leur débuggage.
</ThumbUpAlert>
...
<ThumbDownAlert>
    Contraiement à Cypress, Playwright ne permet pas de continuer la navigation manuellement après l'exécution des tests, ce qui rend le débuggage plus difficile.
</ThumbDownAlert>
...
Après avoir écrit des fonctions auxiliaires chargées du login et du logout pour éviter la duplication de code, on peut utiliser par exemple utiliser les méthodes de Mocha de la manière suivante :
